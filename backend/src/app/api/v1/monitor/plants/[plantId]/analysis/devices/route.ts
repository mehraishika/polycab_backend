...
import { getPlantAnalysisDevices } from '@/server/services/plant.service';
...
import { PlantAnalysisDevicesQueryValidator } from '@/server/validators/plant.validator';
import { UserRepository } from '@/server/repositories/user.repository';

const userRepository = new UserRepository();

type PlantAnalysisDevicesContext = {
	params: Promise<{
		plantId: string;
	}>;
};

async function getPlantAnalysisDevicesRoute(
	request: NextRequest,
	context: PlantAnalysisDevicesContext,
): Promise<Response> {
	...
	const parsedQuery = PlantAnalysisDevicesQueryValidator.safeParse({
	...
	try {
		const data = await getPlantAnalysisDevices({
			scope,
			plantId,
		});

		return successResponse('Analysis device list fetched successfully.', data);
	} catch (error: unknown) {
		if (error instanceof ApiError) {
			return errorResponse(error.message, error.statusCode);
		}
		return errorResponse('Failed to fetch analysis device list', 500);
	}
}

export const GET = withRequestLogging(requireAuth(getPlantAnalysisDevicesRoute), {
	routeName: 'monitor.plants.analysis.devices',
});
