...
} from '@/server/features/remote-settings/feature-parameters/feature-parameters.service';
import {
	featureParametersBodySchema,
	featureParametersQuerySchema,
} from '@/server/features/remote-settings/feature-parameters/feature-parameters.schema';
...
function parseScopeQuery(request: NextRequest) {
	...
	return featureParametersQuerySchema.safeParse({
...
async function getFeatureParametersRoute(request: NextRequest, context: RouteContext): Promise<Response> {
	...
	try {
		const settings = await getFeatureParameters({
			user: buildUser(auth),
			deviceId,
...
async function postFeatureParametersRoute(request: NextRequest, context: RouteContext): Promise<Response> {
	const authenticatedRequest = request as AuthenticatedRequest;
	const auth = authenticatedRequest.auth;
	if (!auth?.userId) return errorResponse('Unauthorized', 401);
	...
	const parsedBody = featureParametersBodySchema.safeParse(body);
	...
	try {
		const result = await submitFeatureParameters({
			user: buildUser(auth),
			deviceId,
			plantId: parsedQuery.data.plantId,
...
export const GET = withRequestLogging(requireAuth(getFeatureParametersRoute), {
	routeName: 'monitor.devices.remoteSettings.featureParameters.get',
});
export const POST = withRequestLogging(requireAuth(postFeatureParametersRoute), {
