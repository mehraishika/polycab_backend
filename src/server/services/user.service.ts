...
type ServiceAdminUserListResult = ServiceAdminUserListSuccess | ServiceResultError;
...
type ServiceAdminUserViewResult = ServiceAdminUserViewSuccess | ServiceResultError;
...
type ServiceAdminUserEditResult = ServiceAdminUserEditSuccess | ServiceResultError;
...
type ServiceAdminUserDeleteResult = ServiceAdminUserDeleteSuccess | ServiceResultError;
...
export class UserService {
	...
	private isRoleAllowedForSubAccountManagement(role: string | undefined): boolean {
	...
	private async validateSubAccountManager(
		...
		if (actor.portal !== 'service' || !this.isRoleAllowedForSubAccountManagement(actor.role)) {
			return {
				status: 403,
				message: 'You are not allowed to manage sub accounts',
			};
	...
	private mapSubAccountStatus(record: { isDeleted: boolean; status: string }): string {
		if (record.isDeleted) {
			return 'deleted';
		}
	...
	async getServiceAdminUserList(
		...
		const accessError = await this.validateSubAccountManager(actorId, actorRole);
		if (accessError) {
			return accessError;
		}

		const result = await this.userRepository.listScopedServiceAdmins(actorId, input);
		const totalPages = Math.max(1, Math.ceil(result.totalItems / input.pageSize));
		const accounts = result.items.map((item) => item.account);
		const deviceCountMap = await this.userRepository.getDeviceCountsByAccounts(accounts);
		...
		return {
			...
			data: {
				items: result.items.map((item) =>
					this.mapSubAccountListItem(item, deviceCountMap.get(item.account) ?? 0),
				),
				pagination: {
					page: input.page,
	...
	async getServiceAdminViewById(
		...
		const record = await this.userRepository.findScopedServiceAdminById(id, actorId);
		...
		return {
			...
			data: this.mapSubAccountViewData(record, deviceCountMap.get(record.account) ?? 0),
		};
	}
	...
	async getServiceAdminList(
		...
		const records =
			await this.userRepository.findScopedServiceAdmins(actorId);
			console.log("records", records);

		const deviceCountMap =
			await this.userRepository.getDeviceCountsByAccounts(
	...
	async editServiceAdminById(
		...
		const existing = await this.userRepository.findScopedServiceAdminById(id, actorId);
		...
		const duplicateEmail = await this.userRepository.findServiceAdminByEmailExcludingId(
			input.email,
			id,
		);
	...
	async deleteServiceAdminById(
		...
		try {
			const deleted = await this.userRepository.softDeleteScopedServiceAdminById(
				id,
				actorId,
			);
	...
	async changePassword(
		...
		const user =
			await this.userRepository.findPasswordById(
				userId
			);
	...
	async forgotPassword(
		...
		const user =
			await this.userRepository.findPasswordByAccount(
				data.account,
			);
