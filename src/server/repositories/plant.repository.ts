  ...
  async getAllPlantsChart(params: {
    ...
    if (params.range === "day") {
      ...
      for (const log of logs) {
        ...
        const current = grouped.get(String(timestamp)) ?? 0;

        grouped.set(
          String(timestamp),
          current + Number(log.total_input_power ?? 0),
        );
      }

      const points = Array.from(grouped.entries()).map(
        ([timestamp, power]) => ({
          time: this.formatDateTime(new Date(Number(timestamp))),
          total: power,
        }),
      );
    ...
    if (params.range === "month") {
      ...
      const points = Array.from({ length: today }, (_, index) => {
        const day = index + 1;
