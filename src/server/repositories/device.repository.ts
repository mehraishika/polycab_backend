...
export class DeviceRepository {
	...
	private mapDeviceRows(devices: Array<any>): DeviceRow[] {
		return devices.map((device) => ({
			...
			today: { value: Number((device.eTodayValue ?? 0).toFixed(2)), unit: 'kWh' },
			total: {
	...
	private sortRows(rows: DeviceRow[], sortBy: DeviceListParams['sortBy'], sortOrder: DeviceListParams['sortOrder']) {
	...
	async getPlantDeviceList(params: DeviceListParams) {
		...
		const [latestLogs, connectionStatuses] =
			await Promise.all([
	...
	async getPlantDeviceLiveRows(params: DeviceLiveRowsParams) {
		...
		const [inverters, dataloggers] = await Promise.all([
			this.dbClient.deviceInverter.findMany({
				where: { id: { in: deviceIds }, plantId: BigInt(params.plantId), deletedAt: null },
				select: {
					id: true,
					name: true,
					type: true,
		...
		return { items: [...this.mapDeviceRows(inverters), ...this.mapDeviceRows(dataloggers)] };
	}
	...
	async getDeviceView(params: DeviceViewParams) {
		...
		return {
			...
			name:
				datalogger.name ??
				`${datalogger.type} ${datalogger.serialNumber}`,
	...
	async editDevice(params: DeviceEditParams) {
		...
		if (inverter) {
			const updated = await this.dbClient.deviceInverter.update({ where: { id: deviceId }, data: { name: params.name, updatedAt: new Date() }, select: { id: true, updatedAt: true } });
			return { id: `device-${String(updated.id)}`, updatedAt: updated.updatedAt.toISOString() };
		}
		...
		const updated = await this.dbClient.deviceDatalogger.update({ where: { id: deviceId }, data: { name: params.name, updatedAt: new Date() }, select: { id: true, updatedAt: true } });
		return { id: `device-${String(updated.id)}`, updatedAt: updated.updatedAt.toISOString() };
	}
	...
	async deleteDevice(params: DeviceDeleteParams) {
		...
		const inverter =
			await this.dbClient.deviceInverter.findFirst({
				where: {
					id: deviceId,
					plantId: BigInt(
						params.plantId,
		...
		if (inverter) {
			const deletedSerial =
				`${inverter.serialNumber}_deleted`;
				...
				await this.dbClient.$transaction(
					async (tx) => {
						...
						const updatedInverter =
							await tx.deviceInverter.update({
								where: {
									id: deviceId,
								},
								data: {
		...
		const deleted =
			await this.dbClient.deviceDatalogger.update({
				where: {
					id: deviceId,
				},
				data: {
	...
	async getDeviceChartSnapshot(params: DeviceChartContextParams): Promise<DeviceChartSnapshot> {
	...
	async getDeviceInformationSnapshot(
		...
		if (inverter) {
			...
			return {
				...
				device: {
					...
					name:
						inverter.name ??
						`${inverter.type} ${inverter.serialNumber}`,
					type: inverter.type,
					sn: inverter.serialNumber,
					...
					communicationModuleSn:
						inverter.dataloggers[0]?.serialNumber ?? 'N/A',
				},
			};
		}
		...
		const inverterUpdatedAt =
			datalogger.inverter?.updateTime ??
			datalogger.inverter?.updatedAt;

		const dataloggerUpdatedAt =
			datalogger.updateTime ??
			datalogger.updatedAt;
		...
		return {
			...
			device: {
				...
				updatedAt:
					inverterUpdatedAt &&
						inverterUpdatedAt > dataloggerUpdatedAt
						? inverterUpdatedAt
						: dataloggerUpdatedAt,
