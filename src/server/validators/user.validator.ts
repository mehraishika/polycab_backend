	...
	const timezoneFormatSchema = z
		.string()
		.trim()
		.regex(/^UTC[+-](0\d|1\d|2[0-3]):[0-5]\d$/, 'Timezone must be in UTC±HH:MM format');
	...
	export const serviceAdminUserListBodySchema = z.object({
		...
		sortBy: z
			.enum(['account', 'email', 'phone', 'timezone', 'status', 'createdAt', 'updatedAt'])
			.default('account'),
	...
	export type ServiceAdminUserListInput = z.infer<typeof serviceAdminUserListBodySchema>;
	export type ServiceAdminEditInput = z.infer<typeof serviceAdminEditBodySchema>;
	export type ServiceAdminDeleteInput = z.infer<typeof serviceAdminDeleteBodySchema>;
