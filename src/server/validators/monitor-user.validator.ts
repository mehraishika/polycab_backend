...
export const monitorUserListQuerySchema = z.object({
	page: z.union([z.number().int().positive(), numberStringSchema]).optional().default(1),
	pageSize: z
		.union([z.number().int().positive().max(100), numberStringSchema])
		.optional()
		.default(10)
		.transform((value) => Math.min(value, 100)),
	status: z.enum(['all', 'normal', 'fault', 'standby', 'offline']).optional().default('all'),
...
export type MonitorUserListQueryInput = z.infer<typeof monitorUserListQuerySchema>;
