...
async function postLogin(request: NextRequest): Promise<Response> {
  ...
  try {
    ...
    return errorResponse("Unable to continue login. Please try again.", 500);
  }
  ...
  if (
    ...
    return successResponse(result.message, {
      ...
      twoFactorChallengeId: result.data.twoFactorChallengeId,
      challengeId: result.data.twoFactorChallengeId,
      verificationMethods: result.data.verificationMethods,
  ...
  if (
    ...
    !("refreshCookieMaxAge" in result.data)
  ) {
    return errorResponse("Authentication tokens were not generated", 500);
  }
