...
import { getPlantAnalysisParameters } from '@/server/services/plant.service';
...
import { PlantAnalysisParametersQueryValidator } from '@/server/validators/plant.validator';
import { UserRepository } from '@/server/repositories/user.repository';

const userRepository = new UserRepository();
type PlantAnalysisParametersContext = {
	params: Promise<{
		plantId: string;
	}>;
};

async function getPlantAnalysisParametersRoute(
	request: NextRequest,
	context: PlantAnalysisParametersContext,
): Promise<Response> {
	...
	const parsedQuery = PlantAnalysisParametersQueryValidator.safeParse({
	...
	try {
		const data = await getPlantAnalysisParameters({
			scope,
			plantId,
			deviceId: parsedQuery.data.deviceId,
		});

		return successResponse('Analysis parameter list fetched successfully.', data);
	} catch (error: unknown) {
		if (error instanceof ApiError) {
			return errorResponse(error.message, error.statusCode);
		}
		return errorResponse('Failed to fetch analysis parameter list', 500);
	}
}

export const GET = withRequestLogging(requireAuth(getPlantAnalysisParametersRoute), {
	routeName: 'monitor.plants.analysis.parameters',
});
