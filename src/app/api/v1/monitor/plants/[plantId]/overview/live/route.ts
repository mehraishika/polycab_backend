...
import { getPlantOverviewLive } from '@/server/services/plant.service';
...
import { PlantOverviewLiveQueryValidator } from '@/server/validators/plant.validator';

type PlantOverviewLiveContext = {
	params: Promise<{
		plantId: string;
	}>;
};

async function getPlantOverviewLiveRoute(
	request: NextRequest,
	context: PlantOverviewLiveContext,
): Promise<Response> {
	...
	const parsedQuery = PlantOverviewLiveQueryValidator.safeParse({
	...
	try {
		const data = await getPlantOverviewLive({
			scope,
			plantId,
			since: parsedQuery.data.since,
		});
		return successResponse('Plant overview live data fetched successfully.', data);
	} catch (error: unknown) {
		...
		return errorResponse('Failed to fetch plant overview live data', 500);
	}
}

export const GET = withRequestLogging(requireAuth(getPlantOverviewLiveRoute), {
	routeName: 'monitor.plants.overview.live',
});
