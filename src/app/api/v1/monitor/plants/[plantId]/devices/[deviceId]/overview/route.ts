...
import { getPlantDeviceOverview } from '@/server/services/plant.service';
import { ApiError } from '@/server/utils/api-error';
import type { User } from '@/server/utils/auth-helper';
import { errorResponse, successResponse } from '@/server/utils/api-response';
import { PlantDeviceOverviewQueryValidator } from '@/server/validators/plant.validator';

type PlantDeviceOverviewContext = {
	params: Promise<{
		plantId: string;
		deviceId: string;
	}>;
...
async function getPlantDeviceOverviewRoute(
	request: NextRequest,
	context: PlantDeviceOverviewContext,
): Promise<Response> {
	...
	const parsedQuery = PlantDeviceOverviewQueryValidator.safeParse({
	...
	try {
		const data = await getPlantDeviceOverview({
			user: buildUser(auth),
			plantId,
			deviceId,
		...
		return successResponse('Device overview fetched successfully.', data);
	} catch (error: unknown) {
		...
		return errorResponse('Failed to fetch device overview', 500);
	}
}

export const GET = withRequestLogging(requireAuth(getPlantDeviceOverviewRoute), {
	routeName: 'monitor.plants.devices.overview',
});
