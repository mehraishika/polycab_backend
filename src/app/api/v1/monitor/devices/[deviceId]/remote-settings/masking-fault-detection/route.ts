...
} from '@/server/features/remote-settings/masking-fault-detection/masking-fault-detection.service';
import {
	maskingFaultDetectionBodySchema,
	maskingFaultDetectionQuerySchema,
} from '@/server/features/remote-settings/masking-fault-detection/masking-fault-detection.schema';
...
function parseScopeQuery(request: NextRequest) {
	...
	return maskingFaultDetectionQuerySchema.safeParse({
...
async function getMaskingFaultDetectionRoute(request: NextRequest, context: RouteContext): Promise<Response> {
	...
	try {
		const settings = await getMaskingFaultDetection({
			user: buildUser(auth),
			deviceId,
...
async function postMaskingFaultDetectionRoute(request: NextRequest, context: RouteContext): Promise<Response> {
	const authenticatedRequest = request as AuthenticatedRequest;
	const auth = authenticatedRequest.auth;
	if (!auth?.userId) return errorResponse('Unauthorized', 401);
	...
	const parsedBody = maskingFaultDetectionBodySchema.safeParse(body);
	...
	try {
		const result = await submitMaskingFaultDetection({
			user: buildUser(auth),
			deviceId,
			plantId: parsedQuery.data.plantId,
...
export const GET = withRequestLogging(requireAuth(getMaskingFaultDetectionRoute), {
	routeName: 'monitor.devices.remoteSettings.maskingFaultDetection.get',
});
export const POST = withRequestLogging(requireAuth(postMaskingFaultDetectionRoute), {
