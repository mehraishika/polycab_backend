...
type GetMonitorUserPlantsContext = {
	params: Promise<{
		monitorUserId: string;
	}>;
};

async function getMonitorUserPlants(
	request: NextRequest,
	context: GetMonitorUserPlantsContext,
): Promise<Response> {
	...
	const parsedParams = monitorUserIdParamSchema.safeParse(params);
	if (!parsedParams.success) {
		return errorResponse('Invalid monitor user id', 400);
	}

	const searchParams = new URL(request.url).searchParams;
	const parsedQuery = monitorUserPlantsQuerySchema.safeParse({
	...
	const result = await monitorUserService.getMonitorUserPlants(
		actorId,
		actorRole,
		parsedParams.data.monitorUserId,
		parsedQuery.data,
	);
...
export const GET = withRequestLogging(requireAuth(getMonitorUserPlants), {
	routeName: 'service.monitor_users.plants',
});
